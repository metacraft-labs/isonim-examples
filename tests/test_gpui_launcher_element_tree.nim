## test_gpui_launcher_element_tree — RS-M11b / EX-M23b headline test.
##
## Drives the *real* ``build/backends/isonim-examples-gpui`` binary via
...
##      true``; an ``element-tree`` M packet arrives before the first F
##      packet. The manifest's ``elements`` array contains the three
##      seeded task rows + a filter bar + a summary entry (the
##      EX-M23b acceptance baseline).
...
##   3. An I packet that resizes the surface (changes the captured
##      ``(width, height)`` inside the launcher) triggers a fresh
...
##
##   1. ``$ISONIM_EXAMPLES_GPUI_BIN``.
##   2. ``<repo-root>/build/backends/isonim-examples-gpui``.
...
# tests/test_tui_launcher_element_tree.nim).
# ---------------------------------------------------------------------------
...
proc resolveLauncherBinary(): string =
  let envPath = getEnv("ISONIM_EXAMPLES_GPUI_BIN")
  if envPath.len > 0:
    if fileExists(envPath): return envPath
    raise newException(IOError,
      "$ISONIM_EXAMPLES_GPUI_BIN points at non-existent file: " & envPath)
  let repoRoot = currentSourcePath().parentDir().parentDir()
  let local = repoRoot / "build" / "backends" / "isonim-examples-gpui"
  if fileExists(local): return local
  raise newException(IOError,
    "isonim-examples-gpui binary not found at " & local & "\n" &
    "Run `just build-backends` in isonim-examples first, or set " &
    "$ISONIM_EXAMPLES_GPUI_BIN to a built binary.")
...
suite "EX-M23b / RS-M11b: GPUI launcher element-tree":
            ...
            if isElementTreeBody(meta.json):
              ...
              if not helloSeen:
                ...
                doAssert helloJson["capabilities"][
                  ...
                  "GPUI launcher must advertise capabilities.elementTree=true"
                helloSeen = true
          of 'F':
            doAssert false,
  ...
  test "state change (I packet) re-emits the manifest":
    when defined(windows):
      ...
      proc flow(): Future[tuple[seenBefore, seenAfter: int]] {.async.} =
        ...
        let start = epochTime()
        # Resize the surface — the launcher's input sink mutates the
        # captured (width, height), which feeds back into the manifest
        # builder's surface dimensions and shifts every entry's bounds.
        # That changes the bridge's manifestKey hash and forces a
        # re-emission per the RS-M11 cadence rule.
